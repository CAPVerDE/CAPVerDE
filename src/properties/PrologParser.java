...
public class PrologParser extends Parser {
	...
	private String parseArch(Architecture arch) {
		...
		// then add the dependence relations, deduction capabilities, counter and composition
		for (Component c : arch.getCompList()) {
			...
			if (c.getAssociate() != null) {
				// add the composition
				facts.add(parseComp(c, c.getAssociate()));
			}
		}
		// go through all actions and add them as facts
		for (Action a : arch.getAllActions()) {
			// parse the actions in the architecture, one after the other
	...
	private List<Term> dummyFacts() {
		...
		Term termSpotcheck = new CompoundTerm(AtomTerm.get("spotcheck"), list3);
		list.add(termSpotcheck);
		...
		Term termComposed = new CompoundTerm(AtomTerm.get("composed"), list2);
		list.add(termComposed);
		return list;
	}
	...
	private Term parseComp(Component comp, Component associate) {
		// TODO
		Term[] args = {AtomTerm.get(comp.getName()), AtomTerm.get(associate.getName())};
		return new CompoundTerm(AtomTerm.get("composed"), args);
	}
	...
	private Term parseAction(Action a) {
		...
		case HAS:
			...
			break;
		case RECEIVE:
			//TODO testing with different lists
		...
		case SPOTCHECK:
			//TODO test this
			Term[] varargs2 = new Term[a.getVarSet().size() + 1];
			int k = 0;
			for (Variable v : a.getVarSet()) {
				varargs2[k] = AtomTerm.get(v.toString());
				k++;
			}
			varargs2[k] = AtomTerm.get("[]");
			Term varlist2 = new CompoundTerm(AtomTerm.get("."), varargs2);
			Term[] argsS = {AtomTerm.get(a.getComponent().toString()), AtomTerm.get(a.getComPartner().toString()), varlist2};
			term = new CompoundTerm(AtomTerm.get("spotcheck"), argsS);
			break;
		case TRUST:
